...
{
    // Get the selected extension from the radio group
    AnsiString selectedExt = RadioGroup2->Items->Strings[RadioGroup2->ItemIndex];
    // Clear the second list box before adding filtered items
    ListBox2->Clear();
    ...
    else // Otherwise, filter items based on the selected extension
    {
        for (int i = 0; i < ListBox1->Items->Count; i++)
        {
            ...
            // Split the line into fields using a custom SplitString function
            std::vector<AnsiString> fields = SplitString(line, "\t");
            // If the extension field matches the selected extension, add the line to ListBox2
    ...
    // Update the label with the selected extension
    lblExt->Caption = "Extension: " + selectedExt;
    // Update the status bar with the number of filtered records
